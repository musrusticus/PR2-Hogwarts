...
public class Main {
	...
	public static List<MagicItem> filterItems(List<MagicItem> items, int digit) {
        List<MagicItem> result = new ArrayList<>();

        for (MagicItem item : items) {
            Selectable selectable = () -> item.getPrice();

            if (filterSelect(selectable, digit)) {
                result.add(item);
            }
        }
    ...
    public static Potion[] sortPotions(List<Potion> potions) {
        ...
        List<Potion> sorted = new ArrayList<>(potions);
        ...
        sorted.sort((p1, p2) -> {
            ...
            // 2. Gewicht + Preis aufsteigend
            int v1 = p1.getWeight() + p1.getPrice();
            int v2 = p2.getWeight() + p2.getPrice();
            return Integer.compare(v1, v2);
        });

        return sorted.toArray(new Potion[0]);
    }
	...
	public static Map<Integer, Set<Wizard>>
			mapWizardsPerNoSpells(List<Wizard> wizards) {
				Map<Integer, Set<Wizard>> result = new HashMap<>();

		        for (Wizard w : wizards) {
		            int n = w.getNumberOfKnownSpells();
		
		            result.computeIfAbsent(n, k -> new HashSet<>())
		                  .add(w);
		        }
